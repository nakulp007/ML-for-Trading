...
def get_portfolio_value(prices, allocs, start_val=1):
    ...
    """
    normed = prices/prices.ix[0,:] #normalized values so all colums start with 1.0
    alloced = normed * allocs #normalized percent alocated for each stock
    pos_vals = alloced * start_val #alocated total money in each stock
    port_val = pos_vals.sum(axis=1) #sum of all invested stocks values each day = portfolio value
    return port_val
...
def get_portfolio_stats(port_val, daily_rf=0, samples_per_year=252):
    ...
    """
    daily_rets = (port_val/port_val.shift(1)) - 1 #calculate daily return. difference in portfolio value each day
    daily_rets = daily_rets[1:] #remove first row of daily portfolio value as the return is always 0
    cum_ret = (port_val[-1]/port_val[0])-1
    avg_daily_ret = daily_rets.mean()
    std_daily_ret = daily_rets.std()
    sharpe_ratio = np.sqrt(samples_per_year) * ( (daily_rets - daily_rf).mean() / (std_daily_ret) )
