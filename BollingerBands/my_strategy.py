...
def calculateBollingerBands(stock_symbol, start_date, end_date):
    ...
    #20day simple moving average
    df_SMA = pd.rolling_mean(df_stock_price, 20, min_periods=20).rename(columns={stock_symbol: 'SMA'})

    #moving standard deviation
    df_moving_std = pd.rolling_std(df_stock_price, 20, min_periods=20).rename(columns={stock_symbol: 'MSTD'})
    #calculate bollinger bands
    bollinger_upperband = (df_SMA['SMA'] + (1*df_moving_std['MSTD']))
    bollinger_lowerband = df_SMA['SMA'] - (1*df_moving_std['MSTD'])
    bollinger_upperband2 = (df_SMA['SMA'] + (2*df_moving_std['MSTD']))
    bollinger_lowerband2 = df_SMA['SMA'] - (2*df_moving_std['MSTD'])
    df_bollinger_bands = pd.concat([bollinger_upperband, bollinger_lowerband, bollinger_upperband2, bollinger_lowerband2], axis=1).rename(columns={0: 'Upper Bollinger Band', 1: 'Lower Bollinger Band', 2: 'Upper Bollinger Band2', 3: 'Lower Bollinger Band2'})
    #.apply(tuple, axis=1)#
...
def computeTrades(df_all):
    ...
    for index, row in df_all.iterrows():
        ...
        if prev_index == -1:
            ...
            if (not inPosition) and (df_all.loc[prev_index,'Stock'] < df_all.loc[prev_index,'Lower Bollinger Band2']) and (df_all.loc[index,'Stock'] > df_all.loc[index,'Lower Bollinger Band2']) and (df_all.loc[index,'Stock'] < df_all.loc[index,'Lower Bollinger Band']):
                df_all.loc[index, 'Bollinger Trades'] = 1
                inPosition = True
            #check if time to exit long position
            elif (inPosition) and (df_all.loc[prev_index,'Stock'] < df_all.loc[prev_index,'Lower Bollinger Band']) and (df_all.loc[index,'Stock'] > df_all.loc[index,'Lower Bollinger Band']):
                df_all.loc[index, 'Bollinger Trades'] = 2
                inPosition = False
            #check if entering from above bollinger band, enter short position
            elif (not inPosition) and (df_all.loc[prev_index,'Stock'] > df_all.loc[prev_index,'Upper Bollinger Band2']) and (df_all.loc[index,'Stock'] < df_all.loc[index,'Upper Bollinger Band2']) and (df_all.loc[index,'Stock'] > df_all.loc[index,'Upper Bollinger Band']):
                df_all.loc[index, 'Bollinger Trades'] = 3
                inPosition = True
            #check if time to exit short position
            elif (inPosition) and (df_all.loc[prev_index,'Stock'] > df_all.loc[prev_index,'Upper Bollinger Band']) and (df_all.loc[index,'Stock'] < df_all.loc[index,'Upper Bollinger Band']):
...
def plotAll(stock_symbol, df_all):
    ...
    df_all['Lower Bollinger Band'].plot(color="cyan", label='')
    df_all['Upper Bollinger Band2'].plot(color="cyan", label='')
    ax = df_all['Lower Bollinger Band2'].plot(color="cyan", label='')
